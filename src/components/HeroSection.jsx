...
const HeroSection = () => {
  ...
  return (
    <div className="relative w-full rounded-3xl sm:rounded-4xl overflow-hidden shadow-2xl bg-slate-950 min-h-[440px] sm:min-h-[480px] flex items-center border border-slate-800/80">
      {/* Background Image */}
      <img
        src={heroBg}
        alt="Culinary Feast Spread"
      ...
      {/* Content Container */}
      <div className="relative z-10 w-full max-w-3xl px-6 py-10 sm:px-12 sm:py-16 space-y-6 sm:space-y-7">
        {/* Live Badge */}
        ...
        <div className="space-y-3">
          ...
          <p className="text-slate-300 text-sm sm:text-base leading-relaxed max-w-xl font-medium">
            Explore authentic dishes across Tamil Nadu, Indian regional cuisines, and world specialties with exact measurements, step-by-step cooking methods, and video tutorials.
          </p>
        </div>
        ...
        <div ref={searchContainerRef} className="relative max-w-xl pt-1">
          ...
          >
            ...
            <input
              ...
              placeholder="Search recipes (e.g. Biryani, Masala Dosa, Pasta, Curry)..."
          ...
          {/* Live Suggestions Dropdown (No Prices, Only Recipe Details) */}
            ...
            <div className="absolute left-0 right-0 top-full mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 text-slate-900 animate-in fade-in slide-in-from-top-2 duration-150">
              ...
              {suggestions.length > 0 ? (
                <div className="divide-y divide-slate-100 max-h-72 overflow-y-auto">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {meal.dietType && (
                            <span
                              className={`shrink-0 text-[10px] font-black px-1.5 py-0.5 rounded-sm border ${
                                meal.dietType === "veg"
                                  ? "text-emerald-700 bg-emerald-50 border-emerald-200"
                                  : "text-rose-700 bg-rose-50 border-rose-200"
                              }`}
                            >
                              {meal.dietType === "veg" ? "VEG" : "NON-VEG"}
                            </span>
                          )}
                        </div>
                        <p className="text-xs text-slate-500 truncate mt-0.5 font-medium">
                          {meal.strCategory} • {meal.strArea || "Special"} Cuisine
                        </p>
                      </div>
                  ...
                  >
                    <span>View all matching recipes for "{searchTerm}"</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                </div>
              ) : !isSearching ? (
                <div className="p-6 text-center text-xs text-slate-500 font-medium">
                  No recipes found matching "{searchTerm}". Try searching for "Dosa", "Biryani", "Curry", or "Pasta".
                </div>
              ) : null}
            </div>
          )}
        ...
        <div className="pt-3 flex flex-wrap items-center gap-6 sm:gap-9 text-xs font-semibold text-slate-300 border-t border-white/10">
          <div className="flex items-center gap-2">
            ...
            <span>Exact Proportions & Measurements</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span>Step-by-Step Cooking Methods</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span>Tamil & World Cuisines via TheMealDB</span>
          </div>
        </div>
      </div>
    </div>
